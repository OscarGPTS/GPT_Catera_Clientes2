            ...
            <div class="px-4 py-3">
                <div class="relative">
                    ...
                    <input
                        ...
                        class="w-full rounded-lg border border-slate-200 bg-slate-50 py-2 pl-8 pr-3 text-[13px] text-slate-900 placeholder:text-slate-400 focus:border-gpt-600 focus:outline-none"
                        autofocus
                    >
                </div>
                <div class="mt-2 max-h-48 overflow-y-auto">
                    ...
                    <button wire:click="startDm({{ $user['id'] }})" class="flex w-full items-center gap-2.5 rounded-lg px-2 py-1.5 text-left hover:bg-gpt-50 transition-colors">
                        <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-gpt-600 text-[9px] font-semibold text-white">
    ...
    <div class="flex w-72 shrink-0 flex-col border-r border-slate-800 bg-slate-900">
        ...
        <div class="flex items-center justify-between border-b border-slate-800/80 px-4 py-3">
            ...
            <div class="flex items-center gap-1">
                <button wire:click="$set('showNewDm', true)" class="rounded-md p-1.5 text-slate-400 hover:bg-slate-800 hover:text-white" title="Nuevo mensaje directo">
    ...
    <div class="flex flex-1 flex-col bg-slate-50">
        @if($activeChannelId)
            ...
            <div
                ...
                x-on:scroll="if ($el.scrollTop === 0 && $el.scrollHeight > $el.clientHeight) { $wire.loadMoreMessages().then(() => { const prevHeight = $el.scrollHeight; $nextTick(() => { $el.scrollTop = $el.scrollHeight - prevHeight + $el.clientHeight; }); }); }"
            >
                {{-- Load more button --}}
                @if($hasMoreMessages)
                <div class="flex justify-center py-2 mb-2">
                    <button
                        ...
                        class="text-[12px] text-gpt-600 hover:text-gpt-700 hover:underline"
                    >
                        ...
                        <div class="group ml-9">
                            ...
                            <div class="mb-1 ml-0">
                                <form wire:submit="updateMessage" class="flex items-end gap-2">
                                    <div class="flex-1">
                                        <textarea wire:model.live="editingContent" rows="2" class="w-full resize-none rounded-lg border border-gpt-600 bg-white px-3 py-2 text-[13px] text-slate-900 focus:outline-none focus:ring-1 focus:ring-gpt-600">{{ $editingContent }}</textarea>
                                    </div>
                        ...
                        <div class="group mt-4 first:mt-0">
                            ...
                            <div class="flex items-start gap-2.5">
                                ...
                                <div>
                                    ...
                                    <p class="mt-1 text-[12px] italic text-slate-400">Este mensaje fue eliminado</p>
                                </div>
                            </div>
                            @else
            ...
            <div class="relative border-t border-slate-200 bg-white px-5 py-3">
                ...
                @endif
                <form wire:submit="sendMessage" class="flex items-end gap-2.5" x-data="{ dragOver: false }"
                ...
                >
                    <div x-show="dragOver" x-transition class="absolute inset-0 z-10 flex items-center justify-center rounded-xl border-2 border-dashed border-gpt-500 bg-gpt-50/80 pointer-events-none">
                        <div class="text-center">
                            <svg class="mx-auto h-8 w-8 text-gpt-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/></svg>
                            <p class="mt-1 text-sm font-medium text-gpt-700">Soltar archivos aquí</p>
                        </div>
                    </div>
                    ...
                    <div class="flex-1 relative" x-data="mentionAutocomplete()">
                        ...
                        <div
                            ...
                            class="absolute bottom-full left-0 mb-1 z-50 w-56 rounded-lg border border-slate-200 bg-white shadow-lg max-h-40 overflow-y-auto"
                        ...
                        >
                            <template x-for="(member, idx) in members" :key="member.id">
                                <button
                                    ...
                                    class="flex w-full items-center gap-2 px-3 py-1.5 text-[12px]"
                                >
                                    <span class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-gpt-600 text-[8px] font-semibold text-white" x-text="member.name.substring(0, 2).toUpperCase()"></span>
    ...
    <script>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('mentionAutocomplete', () => ({
                ...
                handleInput(event) {
                    ...
                    if (atIndex !== -1) {
                        ...
                        if (textAfterAt.length <= 30 && !textAfterAt.includes(' ') && !textAfterAt.includes('\n')) {
                            this.query = textAfterAt.toLowerCase();
                            this.atPosition = atIndex;
                            const component = Livewire.getByName('chat.chat-drawer')[0] || Livewire.getByName('chat.chat-panel')[0];
                            if (component) {
                                const allMembers = component.channelMembers || [];
                                this.members = allMembers.filter(m => m.name.toLowerCase().includes(this.query));
                ...
                navigateDown() {
                    if (this.selectedIndex < this.members.length - 1) this.selectedIndex++;
                },
                ...
                selectMention(event) {
                    if (this.showDropdown && this.members.length > 0) {
                        event.preventDefault();
                        event.stopPropagation();
                        this.insertMention(this.members[this.selectedIndex].name);
                        return;
                    }
                ...
                insertMention(name) {
                    const textarea = this.$refs.chatInput || document.getElementById('chat-input') || document.getElementById('chat-input-drawer');
                    if (!textarea) return;
                    const value = textarea.value;
                    const before = value.substring(0, this.atPosition);
                    const after = value.substring(textarea.selectionStart);
                    const newValue = before + '@' + name + ' ' + after;
                    this.$wire.newMessage = newValue;
                    this.showDropdown = false;
                    this.$nextTick(() => {
                        const newPos = this.atPosition + name.length + 2;
