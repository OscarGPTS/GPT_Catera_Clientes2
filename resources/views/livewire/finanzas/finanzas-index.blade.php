<div>
                ...
                <div class="mt-3 space-y-1">
                    @foreach($cuentas as $cuenta)
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-slate-600">{{ $cuenta->banco ?? '—' }} ••{{ substr($cuenta->numero_cuenta_enmascarado ?? $cuenta->numero ?? '—', -4) }}</span>
    ...
    <div class="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <x-stat-card
            title="Saldo total"
            :value="'$ ' . number_format($saldoTotal, 2, '.', ',')"
            color="blue"
        />
        <x-stat-card
        ...
        <x-stat-card
            ...
            :value="'$ ' . number_format($egresosMes, 2, '.', ',')"
            color="red"
        />
        <x-stat-card
            title="Por conciliar"
            :value="(string) $porConciliar"
            :color="$porConciliar > 0 ? 'amber' : 'green'"
        />
    </div>
</div>