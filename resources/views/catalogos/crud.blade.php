<x-layouts.app>
    ...
    {{--
        Los form-schemas (incluidas las listas de opciones de selects) se pasan
        UNA SOLA VEZ al objeto Alpine, no por fila. Así evitamos el problema de
        68 MB que ocurría al repetir 433 opciones de cliente en cada fila.
    --}}
    @php
        $formSchemas = collect($catalogos)->map(fn($c) => $c['form'])->all();
        $tabLabels   = collect($catalogos)->map(fn($c) => $c['label'])->all();
    @endphp

    <div
        x-data="catalogosCrud(@js($activeTab), @js($formSchemas), @js($tabLabels))"
        class="space-y-4"
    >
        {{-- Flash messages --}}
        @if(session('success'))
        ...
        {{-- Tabs (links de servidor para cambiar de catálogo) --}}
        <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            <div class="flex flex-wrap gap-1 border-b border-slate-200 bg-slate-50/60 p-1.5 overflow-x-auto">
                ...
                @php $isActive = $slug === $activeTab; @endphp
                <a
                    href="{{ route('catalogos.crud.index', ['tab' => $slug]) }}"
                ...
                >
                    ...
                    <span
                        class="inline-flex h-5 min-w-[20px] items-center justify-center rounded-full px-1.5 text-[10px] font-semibold transition-colors
                            {{ $isActive ? 'bg-gpt-100 text-gpt-700' : 'bg-slate-200 text-slate-600' }}"
                    >
            ...
            {{-- Tab content: solo el tab activo tiene registros --}}
            ...
            <div class="flex items-center justify-between border-b border-slate-100 px-5 py-3">
                <div>
                    ...
                    <p class="text-[11px] text-slate-500">
                        {{ number_format($cat['total']) }} registros activos
                        @if($cat['records'] && $cat['records']->lastPage() > 1)
                            · página {{ $cat['records']->currentPage() }}/{{ $cat['records']->lastPage() }}
                        @endif
                    </p>
                </div>
                <button
                    type="button"
                    @click="openCreate(@js($activeTab))"
            ...
            {{-- Table --}}
            @if(!$cat['records'] || $cat['records']->isEmpty())
            ...
            @else
                    ...
                    <table class="min-w-full">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr class="group hover:bg-slate-50/60">
                                @foreach($cat['columns'] as $col)
                                @php $value = data_get($record, $col['key']); @endphp
                                ...
                                <td class="sticky right-0 bg-white px-4 py-2 text-right text-xs group-hover:bg-slate-50/60">
                                    <div class="flex items-center justify-end gap-1.5">
                                        <button
                                            type="button"
                                            @click="openEdit(@js($activeTab), @js($record->toArray()))"
                                        ...
                                        <button
                                            type="button"
                                            @click="confirmDelete(@js($activeTab), {{ $record->id }}, @js($cat['label']))"
                ...
                {{-- Paginación --}}
                @if($cat['records']->lastPage() > 1)
                <div class="flex items-center justify-between border-t border-slate-100 px-5 py-3">
                    <p class="text-xs text-slate-500">
                        Mostrando {{ $cat['records']->firstItem() }}–{{ $cat['records']->lastItem() }}
                        de {{ number_format($cat['records']->total()) }} registros
                    </p>
                    <div class="flex items-center gap-1">
                        {{-- Anterior --}}
                        @if($cat['records']->onFirstPage())
                            <span class="inline-flex items-center rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1.5 text-xs text-slate-400 cursor-not-allowed select-none">
                                ← Ant.
                            </span>
                        @else
                            <a href="{{ $cat['records']->previousPageUrl() }}" class="inline-flex items-center rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-medium text-slate-600 transition-colors hover:bg-slate-50">
                                ← Ant.
                            </a>
                        @endif
                        ...
                        {{-- Páginas cercanas --}}
                        @php
                            $current  = $cat['records']->currentPage();
                            $last     = $cat['records']->lastPage();
                            $window   = range(max(1, $current - 2), min($last, $current + 2));
                        @endphp
                        @if($window[0] > 1)
                            <a href="{{ $cat['records']->url(1) }}" class="inline-flex items-center rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50">1</a>
                            @if($window[0] > 2)<span class="px-1 text-slate-400 text-xs">…</span>@endif
                        @endif
                        @foreach($window as $page)
                            @if($page === $current)
                                <span class="inline-flex items-center rounded-lg border border-gpt-300 bg-gpt-50 px-2.5 py-1.5 text-xs font-semibold text-gpt-700">{{ $page }}</span>
                            @else
                                <a href="{{ $cat['records']->url($page) }}" class="inline-flex items-center rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50">{{ $page }}</a>
                            @endif
                        @endforeach
                        @if(end($window) < $last)
                            @if(end($window) < $last - 1)<span class="px-1 text-slate-400 text-xs">…</span>@endif
                            <a href="{{ $cat['records']->url($last) }}" class="inline-flex items-center rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50">{{ $last }}</a>
                        @endif

                        {{-- Siguiente --}}
                        @if($cat['records']->hasMorePages())
                            <a href="{{ $cat['records']->nextPageUrl() }}" class="inline-flex items-center rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-medium text-slate-600 transition-colors hover:bg-slate-50">
                                Sig. →
                            </a>
                        @else
                            <span class="inline-flex items-center rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1.5 text-xs text-slate-400 cursor-not-allowed select-none">
            ...
            >
                <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                    <div>
                        ...
                        <p class="mt-0.5 text-xs text-slate-500" x-text="tabLabels[formModal.tab] ?? formModal.tab"></p>
                    </div>
        ...
        >
            ...
            >
                ...
                <form :action="deleteAction()" method="POST" class="flex justify-end gap-2 px-6 pb-6 pt-5">
                    ...
                    <button type="button" @click="closeDelete()" class="rounded-lg border border-slate-200 px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50">Cancelar</button>
                    <button type="submit" class="rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-red-700">Sí, eliminar</button>
                </form>
            </div>
        </div>
    </div>
    ...
    <script>
        function catalogosCrud(initialTab, formSchemas, tabLabels) {
            return {
                ...
                submitting: false,
                formModal: {
                    open: false, mode: 'create', tab: null, fields: [], values: {}, id: null,
                },
                deleteModal: {
                    open: false, tab: null, id: null, label: '',
                },
                ...
                openCreate(slug) {
                    this.formModal = {
                        ...
                        tab: slug,
                        fields: this.formSchemas[slug] ?? [],
                        values: {}, id: null,
                    };
                    this.submitting = false;
                },

                openEdit(slug, record) {
                    this.formModal = {
                        open: true, mode: 'edit',
                        tab: slug,
                        fields: this.formSchemas[slug] ?? [],
                ...
                closeForm() { this.formModal.open = false; this.submitting = false; },

                confirmDelete(slug, id, label) {
                    this.deleteModal = { open: true, tab: slug, id, label };
                },
                closeDelete() { this.deleteModal.open = false; },
