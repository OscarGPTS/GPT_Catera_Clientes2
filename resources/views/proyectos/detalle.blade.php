<x-layouts.app>
        ...
        <div class="ml-auto flex items-center gap-2">
            @can('aprobar cp')
                @if($proyecto->estado === 'en_revision')
                    <button type="button" @click="showAprobarModal = true"
                ...
                @endif
            @endcan
            <form method="POST" action="{{ route('oportunidades.cambiar-estado', $proyecto) }}" class="inline" x-data="{ showEstadoModal: false }">
                @csrf
                <button type="button" @click="showEstadoModal = true"
                ...
                <div x-show="showEstadoModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50" @click.self="showEstadoModal = false">
                    <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
                        <h3 class="text-lg font-medium text-slate-900 mb-4">Cambiar estado</h3>
                        <select name="estado" class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-gpt-600 focus:ring-gpt-600 mb-4">
                            @foreach(['en_revision' => 'En revisión', 'cotizando' => 'Cotizando', 'cotizado' => 'Cotizado', 'presentado' => 'Presentado', 'adjudicado_pendiente' => 'Adjudicado pend.', 'adjudicado_firmado' => 'Adjudicado', 'en_ejecucion' => 'En ejecución', 'en_cierre' => 'En cierre', 'cerrado' => 'Cerrado', 'cancelado' => 'Cancelado', 'perdido' => 'Perdido', 'archivado' => 'Archivado'] as $key => $label)
                                <option value="{{ $key }}" {{ $proyecto->estado === $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                        <textarea name="notas" rows="3" placeholder="Notas sobre el cambio de estado..." class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm placeholder:text-slate-400 focus:border-gpt-600 focus:ring-gpt-600 mb-4"></textarea>
                        <div class="flex justify-end gap-3">
                            <button type="button" @click="showEstadoModal = false" class="rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Cancelar</button>
    ...
    {{-- Tabs --}}
    <div class="mb-6 border-b border-slate-200" x-data="{ activeTab: 'info' }">
        <nav class="flex gap-6 -mb-px" aria-label="Tabs">
            <button @click="activeTab = 'info'" :class="activeTab === 'info' ? 'border-gpt-600 text-gpt-600' : 'border-transparent text-slate-500 hover:border-slate-300 hover:text-slate-700'" class="whitespace-nowrap border-b-2 px-1 py-3 text-sm font-medium transition-colors">Información</button>
            <button @click="activeTab = 'equipo'" :class="activeTab === 'equipo' ? 'border-gpt-600 text-gpt-600' : 'border-transparent text-slate-500 hover:border-slate-300 hover:text-slate-700'" class="whitespace-nowrap border-b-2 px-1 py-3 text-sm font-medium transition-colors">Equipo</button>
            <button @click="activeTab = 'cotizaciones'" :class="activeTab === 'cotizaciones' ? 'border-gpt-600 text-gpt-600' : 'border-transparent text-slate-500 hover:border-slate-300 hover:text-slate-700'" class="whitespace-nowrap border-b-2 px-1 py-3 text-sm font-medium transition-colors">Cotizaciones</button>
            <button @click="activeTab = 'solicitudes'" :class="activeTab === 'solicitudes' ? 'border-gpt-600 text-gpt-600' : 'border-transparent text-slate-500 hover:border-slate-300 hover:text-slate-700'" class="whitespace-nowrap border-b-2 px-1 py-3 text-sm font-medium transition-colors">Solicitudes</button>
            <button @click="activeTab = 'eventos'" :class="activeTab === 'eventos' ? 'border-gpt-600 text-gpt-600' : 'border-transparent text-slate-500 hover:border-slate-300 hover:text-slate-700'" class="whitespace-nowrap border-b-2 px-1 py-3 text-sm font-medium transition-colors">Historial</button>
        </nav>
    </div>
                ...
                <div class="rounded-lg border border-slate-200 bg-white p-5">
                    ...
                    @can('asignar cp')
                        <div class="mt-4 pt-3 border-t border-slate-100">
                            <button type="button" @click="showEquipoModal = true" class="text-sm font-medium text-gpt-600 hover:text-gpt-700 transition-colors">Asignar equipo</button>
                        </div>
                    @endcan
                </div>
    ...
    {{-- Tab: Equipo --}}
    <div x-show="activeTab === 'equipo'" class="space-y-6">
        <div class="rounded-lg border border-slate-200 bg-white p-6">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-base font-medium text-slate-900">Equipo del proyecto</h3>
                @can('asignar cp')
                    <button type="button" @click="showEquipoModal = true" class="inline-flex items-center gap-1.5 rounded-lg bg-gpt-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-gpt-700 transition-colors">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                @php
                    $roles = [
                        ['label' => 'Director DN', 'key' => 'director_dn_id', 'user' => $proyecto->directorDn],
                        ['label' => 'Gerente de Proyectos', 'key' => 'gerente_proyectos_id', 'user' => $proyecto->gerenteProyectos],
                        ['label' => 'Gerente de Operaciones', 'key' => 'gerente_operaciones_id', 'user' => $proyecto->gerenteOperaciones],
                        ['label' => 'Ingeniero de Costos', 'key' => 'ingeniero_costos_id', 'user' => $proyecto->ingenieroCostos],
                        ['label' => 'Ingeniero de Proyectos', 'key' => 'ingeniero_proyectos_id', 'user' => $proyecto->ingenieroProyectos],
                        ['label' => 'Trainee', 'key' => 'trainee_id', 'user' => $proyecto->trainee],
                    ];
                @endphp
    ...
    {{-- Tab: Cotizaciones --}}
    <div x-show="activeTab === 'cotizaciones'" class="space-y-6">
    ...
    {{-- Tab: Solicitudes internas --}}
    <div x-show="activeTab === 'solicitudes'" class="space-y-6">
        <div class="rounded-lg border border-slate-200 bg-white overflow-hidden">
            ...
            @else
                <div class="px-6 py-12 text-center">
                    <svg class="mx-auto h-12 w-12 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.25 2.25 0 0113.5 2.25H15m5.323 3.577a48.424 48.424 0 010 18.846"/></svg>
    ...
    {{-- Approve CP Modal --}}
    <div x-show="showAprobarModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50" @click.self="showAprobarModal = false">
        <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
            <h3 class="text-lg font-medium text-slate-900 mb-4">Aprobar CP</h3>
            <form method="POST" action="{{ route('oportunidades.aprobar', $proyecto) }}">
                @csrf
                <div class="space-y-4">
                    <div>
                        <label for="gerente_proyectos_id" class="block text-sm font-medium text-slate-700 mb-1">Gerente de Proyectos</label>
                        <select name="gerente_proyectos_id" id="gerente_proyectos_id" class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-gpt-600 focus:ring-gpt-600">
                            <option value="">Seleccionar...</option>
                            @foreach($equipoDisponible->where('roles', 'gerente_proyectos') as $user)
                    ...
                    <div>
                        <label for="notas" class="block text-sm font-medium text-slate-700 mb-1">Notas (opcional)</label>
                        <textarea name="notas" id="notas" rows="3" class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm placeholder:text-slate-400 focus:border-gpt-600 focus:ring-gpt-600" placeholder="Notas sobre la aprobación..."></textarea>
                    </div>
                </div>
                <div class="mt-6 flex justify-end gap-3">
                    <button type="button" @click="showAprobarModal = false" class="rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Cancelar</button>
                    <button type="submit" class="rounded-lg bg-green-600 px-4 py-2 text-sm font-medium text-white hover:bg-green-700">Aprobar</button>
                </div>
            </form>
        </div>
    </div>
    ...
    {{-- Reject CP Modal --}}
    <div x-show="showRechazarModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50" @click.self="showRechazarModal = false">
        <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
            <h3 class="text-lg font-medium text-slate-900 mb-4">Rechazar CP</h3>
            <form method="POST" action="{{ route('oportunidades.rechazar', $proyecto) }}">
                @csrf
                <div>
                    <label for="rechazar_notas" class="block text-sm font-medium text-slate-700 mb-1">Motivo del rechazo *</label>
                    <textarea name="notas" id="rechazar_notas" rows="4" required class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm placeholder:text-slate-400 focus:border-gpt-600 focus:ring-gpt-600" placeholder="Explique el motivo del rechazo..."></textarea>
                </div>
                <div class="mt-6 flex justify-end gap-3">
                    <button type="button" @click="showRechazarModal = false" class="rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Cancelar</button>
                    <button type="submit" class="rounded-lg bg-gpt-red-600 px-4 py-2 text-sm font-medium text-white hover:bg-gpt-red-700">Rechazar</button>
                </div>
            </form>
        </div>
    </div>
    ...
    {{-- Assign Team Modal --}}
    <div x-show="showEquipoModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50" @click.self="showEquipoModal = false">
        <div class="w-full max-w-2xl rounded-lg bg-white p-6 shadow-xl">
            <h3 class="text-lg font-medium text-slate-900 mb-4">Asignar equipo</h3>
            <form method="POST" action="{{ route('oportunidades.asignar-equipo', $proyecto) }}">
                @csrf
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    @php
                        $teamRoles = [
                            ['label' => 'Director DN', 'key' => 'director_dn_id'],
                            ['label' => 'Gerente de Proyectos', 'key' => 'gerente_proyectos_id'],
                    ...
                    @foreach($teamRoles as $role)
                        <div>
                            ...
                            <select name="{{ $role['key'] }}" id="{{ $role['key'] }}" class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-gpt-600 focus:ring-gpt-600">
                                <option value="">Sin asignar</option>
                                @foreach($equipoDisponible as $user)
                                    <option value="{{ $user->id }}" {{ (string)$proyecto->{$role['key']} === (string)$user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    @endforeach
                ...
                <div class="mt-6 flex justify-end gap-3">
                    <button type="button" @click="showEquipoModal = false" class="rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Cancelar</button>
                    <button type="submit" class="rounded-lg bg-gpt-600 px-4 py-2 text-sm font-medium text-white hover:bg-gpt-700">Guardar equipo</button>
                </div>
            </form>
        </div>
    </div>
