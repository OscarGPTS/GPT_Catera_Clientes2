<x-layouts.app>
    <div class="space-y-6" x-data="asignacionesApp()" x-init="init()">
        {{-- Header --}}
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <div class="flex items-center gap-3">
                    <h2 class="text-2xl font-medium text-slate-900">Reporte de Asignación</h2>
                    <span class="rounded-full bg-gpt-100 px-2.5 py-0.5 text-xs font-medium text-gpt-800">Q{{ $trimestre ?? 1 }} {{ $anio ?? 2026 }}</span>
                </div>
            ...
            <div class="flex flex-wrap items-end gap-3">
                <div>
                    ...
                    <select x-model="filtros.anio" class="mt-1 block w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-gpt-600 focus:ring-gpt-600">
                        @foreach(($anios ?? [date('Y'), date('Y')-1, date('Y')-2]) as $y)
                ...
                <div>
                    ...
                    <select x-model="filtros.trimestre" class="mt-1 block w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-gpt-600 focus:ring-gpt-600">
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700">Gerencia Regional</label>
                    <select x-model="filtros.gerencia" class="mt-1 block w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-gpt-600 focus:ring-gpt-600">
                ...
                <div class="flex gap-2">
                    ...
                    @can('admin')
                    <button type="button" @click="generarSnapshot()" class="inline-flex items-center gap-1.5 rounded-lg bg-gpt-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-gpt-700 transition-colors">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div class="rounded-lg border border-slate-200 bg-white p-4">
                ...
                <p class="mt-2 text-3xl font-semibold text-slate-900">{{ $equipo_total ?? 0 }}</p>
                <p class="mt-1 text-xs text-slate-500">personas</p>
            </div>
            <div class="rounded-lg border border-slate-200 bg-white p-4">
                <p class="text-sm font-medium text-slate-600">Proyectos activos</p>
                <p class="mt-2 text-3xl font-semibold text-slate-900">{{ $proyectos_activos ?? 0 }}</p>
                <p class="mt-1 text-xs text-slate-500">en ejecución</p>
            </div>
            <div class="rounded-lg border border-slate-200 bg-white p-4">
                <p class="text-sm font-medium text-slate-600">Carga promedio</p>
                <p class="mt-2 text-3xl font-semibold text-slate-900">{{ $carga_promedio ?? 0 }}</p>
                <p class="mt-1 text-xs text-slate-500">proyectos / persona</p>
            </div>
            <div class="rounded-lg border {{ ($sobrecarga ?? 0) > 0 ? 'border-gpt-red-200 bg-gpt-red-50' : 'border-slate-200 bg-white' }} p-4">
                <p class="text-sm font-medium {{ ($sobrecarga ?? 0) > 0 ? 'text-gpt-red-700' : 'text-slate-600' }}">Sobrecarga</p>
                <p class="mt-2 text-3xl font-semibold {{ ($sobrecarga ?? 0) > 0 ? 'text-gpt-red-700' : 'text-slate-900' }}">{{ $sobrecarga ?? 0 }}</p>
                <p class="mt-1 text-xs {{ ($sobrecarga ?? 0) > 0 ? 'text-gpt-red-600' : 'text-slate-500' }}">{{ ($sobrecarga ?? 0) > 0 ? 'personas con 10+' : 'sin sobrecarga' }}</p>
            </div>
        </div>

        {{-- Empty state --}}
        @if(empty($snapshot_generado))
        <div class="rounded-lg border border-slate-200 bg-white p-12 text-center">
            ...
            @can('admin')
            <button type="button" @click="generarSnapshot()" class="mt-4 inline-flex items-center gap-1.5 rounded-lg bg-gpt-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-gpt-700 transition-colors">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                Generar snapshot
            </button>
            @endcan
        ...
        {{-- Heatmap Table --}}
        <div class="rounded-lg border border-slate-200 bg-white overflow-hidden" x-data="{ drawerOpen: false, drawerPersona: null, drawerMes: null }">
                                ...
                                @foreach($persona['meses'] ?? array_fill(0, 12, null) as $mi => $carga)
                                    <td class="px-2 py-2.5 text-center">
                                        @if($carga !== null)
                                            ...
                                            @endphp
                                            <button type="button" @click="drawerOpen = true; drawerPersona = {{ $idx }}; drawerMes = {{ $mi }}" class="w-full rounded p-2 text-center text-sm font-medium {{ $colorClass }} hover:ring-2 hover:ring-slate-400 transition-all cursor-pointer">
                                                {{ $carga }}
                                            </button>
                                        @else
            ...
            {{-- Detail drawer --}}
            <div x-show="drawerOpen" x-cloak class="fixed inset-0 z-50 overflow-hidden" x-transition.opacity.duration.300ms>
                <div class="absolute inset-0 bg-slate-900/30" @click="drawerOpen = false"></div>
                <div class="absolute inset-y-0 right-0 flex max-w-full">
                    <div class="w-screen max-w-lg" x-show="drawerOpen" x-transition:enter="transform transition ease-in-out duration-300" x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transform transition ease-in-out duration-300" x-transition:leave-start="translate-x-0" x-transition:leave-end="translate-x-full">
                        <div class="flex h-full flex-col bg-white shadow-xl">
                            <div class="flex items-center justify-between border-b border-slate-200 px-6 py-4">
                                <h3 class="text-lg font-medium text-slate-900">Detalle de asignación</h3>
                                <button @click="drawerOpen = false" class="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors">
                                    <svg class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                                </button>
                            </div>
                            <div class="flex-1 overflow-y-auto p-6" x-data="{ tab: 'actual' }">
                                <template x-if="drawerPersona !== null && $refs.asignacionesData">
                                    <div>
                                        <div class="flex items-center gap-4 mb-6">
                                            <div class="flex h-16 w-16 items-center justify-center rounded-full bg-gpt-100 text-xl font-semibold text-gpt-700">
                                                <span x-text="$refs.asignacionesData[drawerPersona]?.nombre?.substring(0,2).toUpperCase()"></span>
                                            </div>
                                            <div>
                                                <p class="text-xl font-medium text-slate-900" x-text="$refs.asignacionesData[drawerPersona]?.nombre"></p>
                                                <p class="text-sm text-slate-500" x-text="$refs.asignacionesData[drawerPersona]?.puesto"></p>
                                                <p class="text-xs text-slate-400" x-text="'Desde ' + ($refs.asignacionesData[drawerPersona]?.antiguedad || '—')"></p>
                                            </div>
                                        </div>
                                        ...
                                        <div x-show="tab === 'actual'">
                                            <div class="grid grid-cols-2 gap-4">
                                                <div class="rounded-lg border border-slate-200 bg-slate-50 p-3">
                                                    ...
                                                    <p class="mt-1 text-lg font-semibold text-slate-900" x-text="$refs.asignacionesData[drawerPersona]?.cp_asignados || 0"></p>
                                                </div>
                                                <div class="rounded-lg border border-slate-200 bg-slate-50 p-3">
                                                    <p class="text-xs text-slate-500">CP ejecutados</p>
                                                    <p class="mt-1 text-lg font-semibold text-slate-900" x-text="$refs.asignacionesData[drawerPersona]?.cp_ejecutados || 0"></p>
                                                </div>
                                                <div class="rounded-lg border border-slate-200 bg-slate-50 p-3">
                                                    <p class="text-xs text-slate-500">CP remanentes</p>
                                                    <p class="mt-1 text-lg font-semibold text-slate-900" x-text="$refs.asignacionesData[drawerPersona]?.cp_remanentes || 0"></p>
                                                </div>
                                                <div class="rounded-lg border border-slate-200 bg-slate-50 p-3">
                                                    <p class="text-xs text-slate-500">Residual</p>
                                                    <p class="mt-1 text-lg font-semibold text-slate-900" x-text="$refs.asignacionesData[drawerPersona]?.residual || 0"></p>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div x-show="tab === 'rh'">
                                            <dl class="space-y-3">
                                                <div class="flex justify-between border-b border-slate-100 pb-2">
                                                    ...
                                                    <dd class="text-sm font-medium text-slate-900" x-text="$refs.asignacionesData[drawerPersona]?.puesto_rh || '—'"></dd>
                                                </div>
                                                <div class="flex justify-between border-b border-slate-100 pb-2">
                                                    <dt class="text-sm text-slate-500">Ingreso</dt>
                                                    <dd class="text-sm font-medium text-slate-900" x-text="$refs.asignacionesData[drawerPersona]?.fecha_ingreso || '—'"></dd>
                                                </div>
                                                <div class="flex justify-between border-b border-slate-100 pb-2">
                                                    <dt class="text-sm text-slate-500">Formación</dt>
                                                    <dd class="text-sm font-medium text-slate-900" x-text="$refs.asignacionesData[drawerPersona]?.formacion || '—'"></dd>
                                                </div>
                                            </dl>
                                        </div>
                                    </div>
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @forelse($asignaciones ?? [] as $persona)
                            <tr class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="px-4 py-3 text-right text-sm font-medium text-slate-900">{{ $persona['cp_totales'] ?? 0 }}</td>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('asignacionesApp', () => ({
        ...
        generarSnapshot() {
            if (confirm('¿Generar snapshot de asignación para el período actual?')) {
                // Dispatch to Livewire or fetch API
                this.$dispatch('generar-snapshot');
            }
        }
    }));
});
